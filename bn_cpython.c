...
static bn *k_mul(bn *a, bn *b)
{
    ...
    if (kmul_split(a, shift, &ah, &al) < 0)
        goto fail;
    assert(Bn_SIZE(ah) > 0); /* the split isn't degenerate */
    ...
    if (!(t1 = k_mul(ah, bh)))
        goto fail;
    assert(Bn_SIZE(t1) >= 0);
    assert(2 * shift + Bn_SIZE(t1) <= Bn_SIZE(ret));
...
static bn *bn_to_dec(bn *a)
{
    ...
    bn *str = bn_new((bn_size)(maxbits * 0.3010 + 1));
